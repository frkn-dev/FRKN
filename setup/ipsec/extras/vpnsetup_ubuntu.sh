...
prepare_env() {
  bigecho "Prepare VPN configuration..."
  bigecho "You can redefine any variable starts with VPN"
  bigecho "Carefull, subnets should match, you cannot redefine only one variable"

  VPN_L2TP_NET=${VPN_L2TP_NET:-'10.0.0.0/16'}
  VPN_L2TP_LOCAL=${VPN_L2TP_LOCAL:-'10.0.0.1'}
  VPN_L2TP_POOL=${VPN_L2TP_POOL:-'10.0.0.10-10.0.255.250'}
  VPN_XAUTH_NET=${VPN_XAUTH_NET:-'10.1.0.0/16'}
  VPN_XAUTH_POOL=${VPN_XAUTH_POOL:-'10.1.0.10-10.1.255.250'}
  VPN_DNS_SRV1=${VPN_DNS_SRV1:-'8.8.8.8'}  # Google DNS
  VPN_DNS_SRV2=${VPN_DNS_SRV2:-'1.1.1.1'}  # CloudFlare DNS
  SWAN_VER=${SWAN_VER:-'4.7'}
...
check_subnets() {
  if [ -s /etc/ipsec.conf ] && grep -qs "FuckRKN1 VPN script" /etc/sysctl.conf; then
...
update_sysctl() {
  ...
  if ! grep -qs "FuckRKN1 VPN script" /etc/sysctl.conf; then
    conf_bk "/etc/sysctl.conf"
cat >> /etc/sysctl.conf <<EOF

# Added by FuckRKN1 VPN script
kernel.msgmnb = 65536
kernel.msgmax = 65536
